...
## Đã thực hiện (client) — Phần 1 (2026-06-22)

Tổng hợp danh sách giao diện + tính năng client đã code cho Phần 1 (mọi thứ đi qua `agent-core` — GUI không gọi control-plane trực tiếp, A.1.4). Phần 2 **chưa** build (chờ endpoint vendor-side).
...
- `crates/agent-core/src/adapters.rs`: `list_ci_policies` (GET) · `register_ci_policy` (POST upsert) · `delete_ci_policy` (DELETE path catch-all). Thêm `ApiError::Server{status,message}` + helper `expect_ok` → **hiện nguyên văn `error`** từ 400/409 (§1.1). `peers()` reuse cho target picker.
- `gui/src-tauri/src/lib.rs`: 4 command `list_ci_policies` · `add_ci_policy` · `delete_ci_policy` · `list_nodes` (+ struct `CiPolicyDraft`, empty→None để giữ ref XOR environment); đăng ký trong `invoke_handler!`.
...
- **Route `/policies`** — "CI/CD Deploy Rules": list (badge issuer · scope ref/env · target · ), empty-state kèm gợi ý CLI, row tap→edit, nút xoá + confirm dialog, re-fetch sau mutation, hiện nguyên văn lỗi server.
- **Route `/policies/new`** + **`/policies/[...repo]`** (edit, rest param = `owner/name`): dùng chung component `lib/PolicyForm.svelte`. Form: issuer select · repo (readonly khi edit) · scope radio [Ref|Environment] + 1 input · target node dropdown (từ `list_nodes`). Client-validate §1.3 (no wildcard, ref XOR env) — fail thì **không** gọi API.
- **Điều hướng**: sidebar desktop thêm mục "Deploy Rules"; dashboard thêm quick-action "CI/CD Deploy Rules" (để mobile/iOS truy cập được).

### CLI parity (`agent`)
- `crates/agent-daemon/src/ci_policy.rs` + dispatch `ci-policy` trong `main.rs`: `agent ci-policy {list|add|rm}` — cùng adapters §1.5, token qua `--token`/`$ANKAYMA_TOKEN`. GUI và CLI ra **cùng dữ liệu** (§1.7).

### Verify
- `cargo fmt --check` · `cargo clippy --workspace --all-targets -- -D warnings` · `cargo test --workspace` — **xanh**. Clippy desktop **và** `aarch64-apple-ios-sim` đều sạch (tray/desktop gated `#[cfg(desktop)]`).
- **cp.ankayma.com (2026-06-22)**: `GET /api/v1/ci/policy|peers|session` → `401 {"error":"unauthorized"}` (đúng — cần session); `POST /api/v1/ci/policy` (no body) → `422 missing field "issuer"` (endpoint live, đúng shape). Happy-path (đăng nhập GitHub → tạo/sửa/xoá rule) = human QC trên desktop + iOS sim.
- Acceptance §1.8: (1)–(5) logic đã code (re-fetch, client+server validate, target từ `/peers`); (6) 401 → mời đăng nhập lại; (7) **không** có UI role/approve/step-up (tuân guard box).
...
- 2026-06-22 — **Phần 1 implemented** (client): agent-core adapters/domain + Tauri commands + GUI routes `/policies[/new|/[...repo]]` + CLI `agent ci-policy`. Lint/test xanh; cp.ankayma.com endpoints verified live. Xem mục "Đã thực hiện (client)" ở trên.
